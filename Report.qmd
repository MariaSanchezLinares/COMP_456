...
# Ensure LDA model is created
optimal_topics <- 6  # Adjust this based on your optimal topic count
lda_model <- LDA(dtm, k = optimal_topics, control = list(seed = 1234))

# Create JSON for LDAvis visualization
json <- createJSON(
...
output_file <- "ldavis_plot.html"
temp_dir <- tempfile()

if (!dir.exists(temp_dir)) dir.create(temp_dir)
serVis(json, out.dir = temp_dir, open.browser = FALSE)

# Copy the HTML file to a permanent location
file.copy(file.path(temp_dir, "index.html"), output_file, overwrite = TRUE)

# Embed the HTML file in Quarto
includeHTML(output_file)

```

In the previous interactive plot, we recommend establishing the lambda value to be in a 0.5, as it provides the best balance between word frequency in the overall corpus, and it's relevance in the selected topic. However, the toggle is useful to explore the two ends of relevance per given topic.
