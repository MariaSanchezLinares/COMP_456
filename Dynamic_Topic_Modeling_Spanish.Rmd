...
topic_5_articles <- assigned_articles %>% 
  filter(topic == 4) %>% 
  select(ID, title, date, content, source_name, language, length, nchar, processed_text, year)
...
assigned_articles5 <- assigned_articles5 %>%
  left_join(top_terms_list5, by = "topic") %>%
  mutate(contains_top_words = mapply(document_contains_top_words, processed_text, top_words))
...
# Combine both topic-year distributions in one table if needed
final_table5 <- left_join(topic_yearly_distribution5, top_words_yearly_distribution5, by = c("year", "topic"))
