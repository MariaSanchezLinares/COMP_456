...
# serVis(json, out.dir = "lda_vis")
```

```{r}

includeHTML("lda_vis/index.html")
```
...
assigned_articles$top_words <- sapply(assigned_articles$top_words, paste, collapse = ", ")
...
#Visualization
```{r}
ggplot(topic_yearly_distribution, aes(x = year, y = num_docs, fill = as.factor(topic_name))) +
...
```{r}
ggplot(topic_yearly_distribution5, aes(x = year, y = num_docs, fill = as.factor(topic_name))) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +  # "dodge" separates the bars for each topic
  scale_fill_manual(values = c("#1f78b4", "#33a02c", "#e31a1c", "#ff7f00", "#6a3d9a", "#b15928", "#a6cee3", "#b2df8a")) +
  labs(
    title = "Topic Distribution per Year (Count of Papers)",
