...
#   group_by(title)  %>% 
#   summarize(processed_text = str_c(word, collapse = " "), date, title, content, source_name, language, length, main_text, nchar, tokens)
# clean_data
...
clean_data <- ALL_articles %>%
  ...
  mutate(date = mdy(date),
         ...
         month = month(date),
         day = day(date),
         ID = as.character(str_pad(X, width = 4, pad = "0"))
         ) %>% 
